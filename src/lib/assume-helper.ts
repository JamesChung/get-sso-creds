...
export async function assumeRole(roleArn: string, sessionName: string, profile: string): Promise<ICredentials> {
  return new Promise((resolve, reject) => {
    exec(`aws sts assume-role --role-arn ${roleArn} --role-session-name ${sessionName} --profile ${profile}`, (error, stdout, stderr) => {
      if (stderr) {
